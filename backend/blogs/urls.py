...
from .views import (AllBlogsListView, ApplaudDetailView, ApplaudPostView,
                    ...
                    ReadingListPostView, SearchBlogView, UserBlogsListView,changeVisible ,makeFalse,makeTrue)


urlpatterns = [
    # Blog urls
    path('<uuid:blog_id>/changevisible/',changeVisible.as_view(), name="changeVisible blog"),
    path('<uuid:blog_id>/makeTrue/',makeTrue.as_view(), name="makeTrue blog"),
    path('<uuid:blog_id>/makeFalse/',makeFalse.as_view(), name="makeFalse blog"),
