...
class AllBlogsListView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        if category:
            blogs = Blog.objects.filter(status='publish', category=category, visible=True)
        else:
            blogs = Blog.objects.filter(status='publish', visible=True)
...
class changeVisible(APIView):
    def put(self, request: Request, blog_id: uuid) -> Response:
        try:
            ...
            if blog_serializer.is_valid(raise_exception=True):
                blog_serializer.save()
                return Response(data={'message': 'Blog vis successfully', 'blog': blog_serializer.data}, status=status.HTTP_200_OK)
