...
export const EditBlog = (): JSX.Element => {
  ...
  const submit = async (e: React.MouseEvent<HTMLButtonElement>, status: string): Promise<void> => {
    e.preventDefault();

    if (validateFormData()) {
      setIsLoading(true);

      // Call API before editing the blog
      // try {
      //   await fetch(`http://127.0.0.1:8000/api/blogs/${blogId}/changevisible/`, {
      //     method: 'PUT',
      //     headers: {
      ...
      // Call API after editing the blog
      try {
        await fetch(`http://127.0.0.1:8000/api/blogs/${blogId}/makeTrue/`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`,
        ...
        console.error('Error calling API after editing the blog:', error);
        toast.error('Error calling API after editing the blog');
      }
    }
  };
...
//       // Call API before editing the blog
//       try {
//         await fetch(`your-api-url/${blogId}/before`, {
//           method: 'PUT',
//           headers: {
//             'Content-Type': 'application/json',
//             'Authorization': `Bearer ${authToken}`,
...
//       // Call API after editing the blog
//       try {
//         await fetch(`your-api-url/${blogId}/after`, {
//           method: 'PUT',
//           headers: {
//             'Content-Type': 'application/json',
//             'Authorization': `Bearer ${authToken}`,
