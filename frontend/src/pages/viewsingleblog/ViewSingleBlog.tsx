...
export const ViewSingleBlog = (): JSX.Element => {
  ...
  } = useGetBlogQuery(blogId!, authToken);
  const { data: totalComments } = useGetTotalCommentsQuery(blogId!, authToken);
  const { data: whetherUserApplauded } = useWhetherUserApplaudedQuery(
    blogId,
    authToken
  );
  const { data: whetherUserSavedBlog } = useWhetherUserSavedBlogQuery(
    blogId,
    authToken
  );
  ...
  const editBlogOnClick = async () => {
    try {
      await fetch(`http://127.0.0.1:8000/api/blogs/${blogId}/makeFalse/`, {
        method: "PUT",
        headers: {
      ...
      console.error("Error calling API before editing the blog:", error);
      toast.error("Error calling API before editing the blog");
    }
  };
    ...
    <div className="mx-auto max-w-[1080px] mt-20 w-full">
      {isLoadingFetchBlog ? (
        ...
        <>
          ...
          <div className="w-full sm:h-[18rem] p-2">
            <img
              src={`http://127.0.0.1:8000/${blogData?.cover_image}`}
              alt="coverimage"
          ...
          <div className="flex justify-center p-2 items-center">
            ...
            {/* <div className="flex justify-center items-center pl-2">
              ...
              <div className="flex justify-center items-center pl-2">
  <img
    src={`http://127.0.0.1:8000${blogData?.author_profile_image}`}
    alt="author-profileimage"
    className="w-7 h-7 sm:w-10 sm:h-10 rounded-full"
  />
              ...
              <p className=" text-sm p-2">
                @ {blogData?.["author_username"] + " • "}
              </p>
              <p className="text-center text-[0.5rem] sm:text-[0.8rem]">
            ...
            <div className="p-4 flex justify-end gap-x-2">
              <div
                onClick={applaudBlogOnClick}
                className="flex flex-col justify-between items-center p-2 cursor-pointer hover:scale-110 duration-300"
              >
                <HiFire
                  size={18}
                  color={`${
                    whetherUserApplauded?.message === "true" ? "#c095e8" : ""
                  }`}
                />
              ...
              <div
                onClick={openCommentsModalOnClick}
                className="flex flex-col justify-between items-center p-2 cursor-pointer hover:scale-110 duration-300"
              >
                <FaComments />
                <p className="text-[0.62rem]">{totalComments?.total}</p>
              ...
              <div
                onClick={saveBlogOnClick}
                className="flex flex-col justify-between items-center p-2 cursor-pointer hover:scale-110 duration-300"
              >
                <BsFillBookmarkCheckFill
                  color={`${
                    whetherUserSavedBlog?.message === "true" ? "#c095e8" : ""
                  }`}
                />
              </div>
            </div>
          ...
          {blogData?.author === userId ? (
            <div className="flex justify-center items-center">
              <AiFillEdit
                onClick={editBlogOnClick}
                className="hover:scale-110 duration-300 cursor-pointer w-8 h-8 bg-yellow-300 rounded-full p-1"
              />
            </div>
          ) : null}

          {/** Title & subtitle */}
          <h2 className="mt-4 p-2 font-bold text-sm text-center sm:text-lg break-words">
            {blogData?.title}
          </h2>
          <p className="p-2 text-[0.55rem] sm:text-[0.75rem] text-center break-words">
            {blogData?.subtitle}
          </p>
